...
>
> 2、金额单位为<span style="color:red;"> USDT（USDT） </span>，接口中的金额字段均按 USDT 金额 x100 传值。
>
> 3、请求接口的 IP 需要加白。
>
...
| price          | int    | true  | 下单金额，单位：USDT（USDT），整型，实际传值为 USDT 金额 x100 |
...
| <span style="color:red;">real_price</span>   | int    | true  | <span style="color:red;">用户真实付款金额,单位:USDT（USDT），实际传值为 USDT 金额 x100</span>                                                                                                                                                               |
| nti_time     | int    | false | 发起通知时间                                                                                                                                                                                   |
| payer        | string | false | JSON 字符串,付款人信息{"account":"账号"}，除示例字段外，当前参数会整合商户传递的attach里付款人信息相关字段 |
...
| trade_no     | int    | true  | 商户号                                                                                                                                               |
| <span style="color:red;">real_price</span>   | int    | true  | <span style="color:red;">真实付款金额,单位:USDT（USDT），实际传值为 USDT 金额 x100</span>                                                                                                                              |
...
| code    | int    | true | 200:查询成功 其他:失败      |
| msg     | string | true | 失败原因                    |
| balance | int    | true | 余额,单位:USDT（USDT），实际传值为 USDT 金额 x100       |
| balance_frozen | int    | false | 冻结余额,单位:USDT（USDT），实际传值为 USDT 金额 x100 |
| sign    | string | true | 签名结果,签名方法在文档顶部 |
...
| order_no       | string | true  | 商户订单号                     |
| amount       | string | true  | 商户金额，单位USDT（USDT），实际传值为 USDT 金额 x100                     |
