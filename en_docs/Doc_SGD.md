...
| <span style="color:red;">real_price</span> | int | false | <span style="color:red;">Actual pay-out amount (only available when the pay-out succeeds). This field has not yet been updated. The signature verification algorithm should account for this field during integration to avoid signature errors after the update.</span> |
